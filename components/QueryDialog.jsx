...
function QueryDialog({ queryDialog, setQueryDialog }) {
  ...
  return (
    <Dialog open={queryDialog} onOpenChange={setQueryDialog} className="">
      <DialogContent className="bg-cblack text-white ">
        <DialogHeader className={` text-white`}>
          <DialogTitle>Query Dialog</DialogTitle>
        </DialogHeader>
        <Tabs defaultValue="NameStyle" className="w-[400px] text-white">
          <TabsList className="p-0 bg-transparent gap-5 mb-3">
            <TabsTrigger
              className="text-md border data-[state=active]:bg-primary text-yellow-100"
              value="NameStyle"
            >
              Name Style
            </TabsTrigger>
